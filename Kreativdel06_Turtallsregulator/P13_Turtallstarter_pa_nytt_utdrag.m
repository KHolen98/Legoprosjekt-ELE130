...
onsketFart = 0; % Ønsket hastighet basert på JoyPot
forrige_feilA = 0; % Forrige feil for motor A
forrige_feilB = 0; % Forrige feil for motor B
integralFeilA = 0; % Integrert feil for motor A
integralFeilB = 0; % Integrert feil for motor B
PID_A = 0;
PID_B = 0;
...
while ~JoyMainSwitch
    if online
        ...
        if k > 1
           ...
           Ts = Tid(k) - Tid(k-1); % Beregn tidsdifferanse
            % Anta at JoyPot endrer ønsket hastighet lineært
            onsketFart = JoyPot(k) * maxPower; % Juster dette etter behov
                % Beregn faktisk hastighet som endring i vinkelposisjon delt på tid
            maaltfartA = (VinkelPosMotorA(k) - VinkelPosMotorA(k-1)) / Ts;
            maaltfartB = (VinkelPosMotorB(k) - VinkelPosMotorB(k-1)) / Ts;

                % Beregn feilen mellom ønsket og faktisk hastighet
            feilA = onsketFart - maaltfartA;
            feilB = onsketFart - maaltfartB;


             % Akkumuler feilen for integraldelen
            integralFeilA = integralFeilA + feilA * Ts;
            integralFeilB = integralFeilB + feilB * Ts;

            % Beregn endring i feil for derivatdelen
            derivasjonFeilA = (feilA - forrige_feilA) / Ts;
            derivasjonFeilB = (feilB - forrige_feilB) / Ts;

            % Beregn PID-pådrag for hver motor
            PID_A = Kp * feilA + Ki * integralFeilA + Kd * derivasjonFeilA;
            PID_B = Kp * feilB + Ki * integralFeilB + Kd * derivasjonFeilB;
        end
